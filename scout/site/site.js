...
function renderTeam() {
  ...
  // 答え合わせは本編に。戦前に書いた分析は資料なので巻末に畳む
  const pm = k === "城東" ? postmortemSection() : null;
  ...
  if (k === "城東") {
    const plan = planSections({ myths: true, style: true, clock: true, keys: true, risks: true, conditions: true });
    main.append(h("section", { class: "sec appendix", id: "appendix" },
      h("div", { class: "sec-head" }, h("h2", {}, "付  スカウティング記録"), h("p", {}, "試合の前に書いたもの")),
      h("p", { class: "prose small muted" }, "ブロック決勝の前に、城東をどう読んでいたかの記録です。"
        + "結果は上の答え合わせのとおりで、勝ち筋7つは当たりませんでした。外れた読みも残してあります。"),
      h("details", { class: "more" }, h("summary", {}, `戦前の分析を開く（${plan.length}項目）`),
