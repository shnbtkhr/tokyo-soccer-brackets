"""第5地区（NSリーグ・goalnote）の順位表と試合を集める。

部品の決まりは notes/district_collector_spec.md。ページの読み方は fetch_leagues.py の goalnote() を写して使う
（fetch_leagues.py 自体は変更しない）。NSリーグは「1部・2部A・2部B・3部A・3部B・3部C」の見出しがあり、
グループA〜Fがそれぞれ対応する（校名末尾のA/B/C等は同一校の複数チームを示す。data/leagues/sources.csv 確認済み）。
2024年度（tid=15685）の順位表だけ「PK勝利・PK敗戦」の列が追加で挟まるため、列名で位置を決める。

取得したページは raw/web/district/d5/ に保存する。refresh=False なら保存済みのページだけで動く。
"""
...
YEAR_TID = {2024: 15685, 2025: 16999, 2026: 18597}
...
def parse_year(year: int, tid: int, refresh: bool) -> tuple[list[dict], list[dict]]:
    ...
    division: dict[str, str] = {}  # グループ文字（A〜F）→ 部ラベル（1部・2部A など）
    matches: list[dict] = []
    cur_div = ""
    cur_round = ""
